...
class HomepageTemplate implements ExtendableComponentInterface
{
	...
	{
		$references = [];
		$find_section_by_name = function(&$array, $name) {
			$l = count($array);
			for($i = 0; $i < $l; $i++) {
		...
		$users_sec = $find_section_by_name($this->sections, "users"); 
		array_splice($this->sections, $users_sec[0], 1);
		array_splice($this->sections, $find_section_by_name($this->sections, "knowledgebase")[0] + 1, 0, [$users_sec[1]]);
