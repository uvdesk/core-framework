...
class Report extends AbstractController
{
    ...
    public function __construct(UserService $userService, UVDeskService $uvdeskService, ReportService $reportService, PaginatorInterface $paginator, EntityManagerInterface $entityManager)
    {
    ...
    public function getAchievementsXhr(Request $request, ContainerInterface $container)
    {
        $json = array();
        if ($request->isXmlHttpRequest()) {
            $repository = $this->entityManager->getRepository(TicketRating::class);
