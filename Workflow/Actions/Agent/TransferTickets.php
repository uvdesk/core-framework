    ...
    public static function applyAction(ContainerInterface $container, $entity, $value = null)
    {
        ...
        if ($entity instanceof User) {
            if ($value == 'responsePerforming') {
                ...
                $user = $entityManager->getRepository('CoreFrameworkBundle:User')->find($value);
            }
            
            if (!empty($user) && $user != 'anon.') {
                $tickets = $entityManager->getRepository('CoreFrameworkBundle:Ticket')->getAgentTickets($entity->getId(), $container);
